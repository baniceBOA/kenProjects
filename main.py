...
class MainApp(MDApp):
    ...
    def calculate_output(self):
        try:
            ...
            self.root.ids.output_maximum_height.color = 'black'
            self.root.ids.output_maximum_height.text = f"Maximum Height: {max_height:.6f} meters"
            self.root.ids.output_horizontal_distance.text = f"Horizontal Distance: {horiz_distance:.6f} meters"
            self.root.ids.output_time_taken.text = f"Time taken to reach Maximum height: {time_of_flight:.6f} seconds"
        except ValueError:
            self.root.ids.output_maximum_height.color = 'red'
            self.root.ids.output_maximum_height.text = "Please enter valid values for velocity and angle."
